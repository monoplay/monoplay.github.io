...
LAUNCHER_FRONT_ORDER_BASKET.editor = {
    setHtml: function () {
        ...
        option.each(function (index, item) {
            ...
            const target = $(item).closest("div.prdBox").children("div.description").find("a.ec-product-name").prop("href") +
                '/design_id/' + design_id;

            // target url ㅂㅏ꾸기
            $(item).closest("div.prdBox").children("div.description").find("a.ec-product-name").prop("href", target);
